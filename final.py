...
def call_DLS(tmpState,limit):
    ...
    global call
    call = 0
    tmpNode = node(state=tmpState, parent=-1, action=getAction(srtX, srtY), g_cost=0, depth=0, h_cost=0, x=srtX, y=srtY,move="intit",ck=0)
    Node += [tmpNode]
    cutOffNode = node(state=tmpState, parent=-5, action=getAction(srtX, srtY), g_cost=-1, depth=-1, h_cost=-1, x=srtX, y=srtY,move="intit",ck=0)
    isVisited.clear()
