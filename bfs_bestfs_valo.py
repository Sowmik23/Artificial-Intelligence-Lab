...
class Node():
	...
	def BFS(self, final):
		...
		while q:
			...
			if np.array_equal(top.state, final):
				...
				if top.move_up():
					new_state, up_value = top.move_up()
					print("Exploring action up")
					if tuple(new_state.reshape(1,9)[0]) not in visited:
						top.up = Node(state = new_state, parent = top, action ='left', length = current_len + 1, step_cost=up_value, cost = current_cost + up_value, heuristic_cost = 0)
						q.append(top.up)
				...
				if top.move_left():
					new_state, left_value = top.move_left()
					print("Exploring action left")
					if tuple(new_state.reshape(1,9)[0]) not in visited:
						top.left = Node(state = new_state, parent = top, action = 'up', length = current_len + 1, step_cost = left_value , cost = current_cost + left_value, heuristic_cost = 0)
						q.append(top.left)
						q_len.append(current_len + 1)
				...
				if top.move_down():
					new_state, down_value = top.move_down()
					print("Exploring action down")
					if tuple(new_state.reshape(1,9)[0]) not in visited:
						top.down = Node(state = new_state, parent = top, action ='right', length = current_len + 1, step_cost=down_value, cost = current_cost + down_value, heuristic_cost = 0)
						q.append(top.down)
						q_len.append(current_len + 1)
				...
				if top.move_right():
					new_state, right_value = top.move_right()
					print("Exploring action right")
					if tuple(new_state.reshape(1,9)[0]) not in visited:
						top.right = Node(state = new_state, parent = top, action = 'down', length = current_len + 1, step_cost = right_value , cost = current_cost + right_value, heuristic_cost = 0)
						q.append(top.right)
						q_len.append(current_len + 1)
	...
	def uniform_cost_search(self, goal_state):
		...
		while queue:
			...
			print("Current node depth "+ str(current_depth))
			print("Call Count "+str(queue_num_nodes_popped))
			...
			if np.array_equal(current_node.state,goal_state):
				...
				if current_node.move_down():
				    new_state,down_value = current_node.move_down()
				    print("Exploring action down")
				    # check if the resulting node is already visited
				    if tuple(new_state.reshape(1,9)[0]) not in visited:
				        current_node.down = Node(state=new_state,parent=current_node,action='right',length=current_depth, step_cost=down_value,cost=current_path_cost+down_value,heuristic_cost=0)
				        queue.append((current_node.down,current_path_cost+down_value))
				        depth_queue.append((current_depth+1,current_path_cost+down_value))
				        path_cost_queue.append(current_path_cost+down_value)
				    
				# see if moving left tile to the right is a valid move
				if current_node.move_right():
				    new_state,right_value = current_node.move_right()
				    print("Exploring action right")
				    # check if the resulting node is already visited
				    if tuple(new_state.reshape(1,9)[0]) not in visited:
				        # create a new child node
				        current_node.right = Node(state=new_state,parent=current_node,action='down',length=current_depth+1,step_cost=right_value,cost=current_path_cost+right_value,heuristic_cost=0)
				        queue.append((current_node.right,current_path_cost+right_value))
				...
				if current_node.move_up():
				    new_state,up_value = current_node.move_up()
				    print("Exploring action up")
				    # check if the resulting node is already visited
				    if tuple(new_state.reshape(1,9)[0]) not in visited:
				        # create a new child node
				        current_node.up = Node(state=new_state,parent=current_node,action='left',length=current_depth+1,step_cost=up_value,cost=current_path_cost+up_value,heuristic_cost=0)
				        queue.append((current_node.up,current_path_cost+up_value))
				...
				if current_node.move_left():
				    new_state,left_value = current_node.move_left()
				    print("Exploring action left")
				    # check if the resulting node is already visited
				    if tuple(new_state.reshape(1,9)[0]) not in visited:
				        # create a new child node
				        current_node.left = Node(state=new_state,parent=current_node,action='up',length=current_depth+1,step_cost=left_value,cost=current_path_cost+left_value,heuristic_cost=0)
				        queue.append((current_node.left,current_path_cost+left_value))
...
if __name__ == '__main__':
	try:
		...
		# rn.Best_FS(final)
		# print("hehehehehehehe	he Best_FS finished!!!...........>")


		rn.uniform_cost_search(final)
