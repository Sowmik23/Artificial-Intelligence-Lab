...
class Node():
	...
	def move_up(self):
		...
		if index_0[0] == 2:
			...
			down_value = self.state[index_0[0]+1,index_0[1]]
	...
	def move_left(self):
		...
		if index_0[1] == 2:
			...
			new_state = self.state.copy()
			right_value = self.state[index_0[0], index_0[1]+1]
	...
	def move_down(self):
		...
		if index_0[0] == 0:
			...
			new_state = self.state.copy()
			up_value = self.state[index_0[0]-1, index_0[1]]
	...
	def move_right(self):
		...
		if index_0[1] == 0:
			...
			# print("this is test")
			new_state = self.state.copy()
			left_value = self.state[index_0[0], index_0[1]-1]
	...
	def calculate_heuristic_cost(self, new_state, final, h_function, cost, length):
	...
	def print_path(self):
		p_state = [self.state]
		p_action = [self.action]
		p_length = [self.length]
		p_step_cost = [self.step_cost]
		p_cost = [self.cost]
		p_heuristic_cost = [self.heuristic_cost]

		while self.parent:
			self = self.parent
			p_state.append(self.state)
			p_action.append(self.action)
			p_length.append(self.length)
			p_step_cost.append(self.step_cost)
			p_cost.append(self.cost)
			p_heuristic_cost.append(self.heuristic_cost)

		cnt = 0
		while p_state:
			print('step {}'.format(cnt))
			print('action = {} depth= {} replaced tiles= {} total_cost= {}'.format(p_action.pop(), str(p_length.pop()), str(p_step_cost.pop()), str( p_cost.pop() + p_heuristic_cost.pop()) ))
			print(p_state.pop())
			print("\n\n")
			# print(cnt)
			cnt +=1
			...
			if np.array_equal(top.state, final):
				...
				if top.move_up():
					...
					if tuple(new_state.reshape(1,9)[0]) not in visited:
						top.up = Node(state = new_state, parent = top, action ='left', length = current_len + 1, step_cost=down_value, cost = current_cost + down_value, heuristic_cost = 0)
						q.append(top.up)
	...
	def UCS(self, final_state):
		...
		while q:
			...
			q_len = sorted(q_len, key = lambda y: y[1])
